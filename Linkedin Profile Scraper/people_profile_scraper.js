...
async function getProfileData(profileLink) {
    ...
    let [name,headline, location, about, experience, education, licensesCertifications, publications, languages, services, awards] = Array(10).fill('N/A');
    ...
    try {
        ...
        headline = await page.$eval('h2.top-card-layout__headline.break-words.font-sans.text-md.leading-open.text-color-text', el => el.textContent.trim()).catch(() => 'N/A');
        ...
        const education = await page.$eval('section[data-section="educationsDetails"] ul.education__list li:first-child', el => {
            const university = el.querySelector('h3')?.textContent.trim() || 'N/A';
            // Uncomment the following lines if you also want to extract degree and field of study
            ...
            const dateRange = el.querySelector('.date-range')?.textContent.trim() || 'N/A'; // Extracting the date range
        ...
        awards = await page.$$eval('section[data-section="honors-and-awards"] ul li', elements => 
            elements.map(el => {
                const title = el.querySelector('h3')?.textContent.trim() || 'N/A';
                const organization = el.querySelector('h4')?.textContent.trim() || 'N/A';
                const date = el.querySelector('.date-range time')?.textContent.trim() || 'N/A';
                const description = el.querySelector('.show-more-less-text__text--less')?.textContent.trim() || 'N/A';
